...
* [Day   30  (2025-08-14)](#30)
* [Day   31  (2025-08-15)](#31)
* [Day   32  (2025-08-16)](#32)
* [Day   33  (2025-08-17)](#33)
* [Day   34  (2025-08-18)](#34)
* [Day   35  (2025-08-19)](#35)
<span id="0"></span>
...
今天主要完善run_guest的demo实现，参考hvisor和h系列实验的代码，初步实现了LoongArch架构下的context switching逻辑。遇到的主要问题是寄存器保存和恢复的顺序，以及HSTATUS寄存器的配置。通过对比RISC-V的实现，发现LoongArch的虚拟化扩展在某些细节上有所不同，需要仔细阅读架构手册。

另外开始尝试在qemu上运行初步的测试代码，但还没有成功跑起来，明天继续调试。

<span id="31"></span>

### Day32

今天主要调试run_guest的实现，发现之前的代码在处理异常退出时有问题。通过添加详细的日志输出，发现问题出现在客户机第一次执行指令时触发的异常处理上。参考谢助教提供的loongarchvcpu代码，重新设计了异常处理流程。

同时开始学习LoongArch的TLB管理机制，为后续实现二阶段地址翻译做准备。发现LoongArch的TLB结构比其他架构更复杂，需要更多时间理解。

<span id="32"></span>

### Day33

今天重点优化了vcpu的数据结构设计，参考axvcpu其他架构的实现，为LoongArch添加了特有的寄存器状态管理。主要包括：
- CSR寄存器的保存和恢复
- TLB相关寄存器的管理  
- 虚拟化扩展特有寄存器的处理

另外开始编写二阶段地址翻译的相关代码，这部分可以参考axaddrspace的通用接口，但需要针对LoongArch的页表格式做适配。预计明天可以完成基本的地址翻译逻辑。

<span id="33"></span>

### Day34

今天主要完善代码文档和进行代码review。将前几天编写的代码整理成可读性更好的形式，添加了详细的注释说明各个函数的功能和参数。同时开始准备向axvcpu仓库提交PR的相关工作。

下午和谢助教讨论了目前的进展，确认了接下来的开发重点：
1. 完成基本的vcpu实现并通过测试
2. 实现完整的二阶段地址翻译
3. 添加中断虚拟化支持
...
今天主要进行集成测试，将前面实现的各个模块组合起来测试。发现了几个问题：
1. vcpu state的初始化顺序有误，导致客户机启动失败
2. 地址翻译在某些边界情况下会出错
3. 异常处理的恢复路径还需要优化

通过逐步调试修复了大部分问题，现在可以成功加载简单的客户机代码并执行几条指令。虽然还不够稳定，但这是一个重要的里程碑。

接下来的计划是继续完善稳定性，然后开始添加更多的虚拟化功能支持。
